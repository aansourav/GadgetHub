...
const MyProducts = () => {
  ...
  useEffect(() => {
    fetch(`https://stormy-tor-21986.herokuapp.com/readmyGadgetData?email=${email}`)
      .then((res) => res.json())
  ...
  const deleteCar = async (id) => {
    ...
    if (isExecuted) {
      fetch(`https://stormy-tor-21986.herokuapp.com/deleteCarData/${id}`, {
        method: "DELETE",
        headers: {
  ...
  return (
    <div class="container my-5">
      ...
      <div class="sidebar-find-car w-25 mx-auto my-2">
        <div>
          <button className="btn" onClick={() => navigate("/addproducts")}>
            Add New Car
          </button>
        </div>
      </div>
                  ...
                  <tbody>
                    {product.map((product) => {
                      return (
                        <>
                          <tr>
                            <td className="table-img d-flex align-items-center justify-content-center">
