...
const ProductDetails = () => {
  ...
  useEffect(() => {
    fetch(`https://stormy-tor-21986.herokuapp.com/readSingleGadgetData/${id}`)
      .then((res) => res.json())
      .then((data) => {
        setProduct(data);
  ...
  const { _id, name, des, img, price, fuel_Type, year, make, body, mileage, transmission, supplier_name } =
    product;

  const deliverProduct = () => {
    const token = localStorage.getItem("accessToken");
    fetch(`https://stormy-tor-21986.herokuapp.com/deliverCarData/${id}`, {
      method: "PUT",
      headers: { "Content-Type": "application/json", accesstoken: `${email} ${token}` },
    })
      .then((res) => res.json())
      .then((data) => {
  ...
  const handleFormsubmit = (e) => {
    const token = localStorage.getItem("accessToken");
    e.preventDefault();
    fetch("https://stormy-tor-21986.herokuapp.com/updateStock", {
      method: "POST",
      headers: { "Content-Type": "application/json", accesstoken: `${email} ${token}` },
              ...
              <div class="inventory-details-description mb-3">
                ...
                <div class="">
                  <ul className="description-list">
                    ...
                    <li>
                      <strong>Supplier name: {supplier_name}</strong>
                    </li>
                    <li>
                      <strong>Status: {quantity > 0 ? "Stock Available" : "Sold Out"}</strong>
                    </li>
                  </ul>
                </div>
              </div>
              ...
              <aside class="inventory-sidebar">
                <div class="widget inventory-widget mb-3">
                  ...
                  <div class="sidebar-find-car">
                    <div>
                      <button className="btn" onClick={() => navigate("/manageproducts")}>
                        Manage
                      </button>
                    </div>
                  </div>
                ...
                <div class="widget inventory-widget mb-3">
                  ...
                  <div class="sidebar-find-car">
                    {quantity == 0 ? (
                      ...
                      <>
                        <button className="btn" onClick={deliverProduct}>
                          Deliver One
                        </button>
                      </>
                    )}
